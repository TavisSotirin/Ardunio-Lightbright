...
void test()
{
  Serial.println("WIFI INTERRUPT!!");

  WiFiClient client = server.available();
  if (client) {
    Serial.println("New client");
    //matrix.println("New client!");
    //matrix.show();
    ...
    Serial.println("INTERRUPT FAILED TO GRAB CLIENT");
  }
...
void setup() {
  ...
  // Interrupts!SPIWIFI_SS
  //pinMode(ESP32_GPIO0, INPUT_PULLUP);
  //attachInterrupt(digitalPinToInterrupt(ESP32_GPIO0), test, CHANGE);
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            if (curFile)
            {
              ...
              {
                size_t readBytes = curFile.readBytes(buf,1024);
